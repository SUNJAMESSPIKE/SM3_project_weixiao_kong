...
unsigned int _CF(unsigned char* ucpSrcMsg, unsigned int nHash[8])
{
	...
	for (j = 0; j < 64; j++)
	{
		SS1 = _rotate_left_move((_rotate_left_move(A_G[A], 12) + A_G[E] + _rotate_left_move(T_1[j], j % 32)), 7);
...
void int_to_hex_1(int a, char ch[10]) {
	char mid_char;
	int temp = 0, i = 0;
	while (a / 16 > 0)
	{
